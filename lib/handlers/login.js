...
const { getKeypair, getKeypairThumbprint, generateDPoP } = require("../dpop");

const getAuthorizeParams = async (req, extras = {}) => ({
  client_id: getEnv().APP_CLIENT_ID,
  response_type: getEnv().response_type,
  redirect_uri: APP_CALLBACK_URL,
  dpop_jkt: await getKeypairThumbprint(await getKeypair()),
...
const authenticate = async (req, res) => {
  ...
  authorizeUrl.query = await getAuthorizeParams(req);

  res.redirect(url.format(authorizeUrl));
};

const authenticateWithRar = async (req, res) => {
  console.log(
    `Calling ${APP_CALLBACK_URL} with audience ${getEnv().audience} and scope ${
      getEnv().scope
    }`
...
const authenticateWithPar = async (req, res) => {
  ...
  const response = await axios.post(
    ...
    querystring.stringify({
      ...(await getAuthorizeParams(req)),
      client_secret: getEnv().APP_CLIENT_SECRET,
    }),
    {
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
        DPoP: await generateDPoP(PAR_ENDPOINT, "post"),
      },
    }
  );
  const authorizeUrl = url.parse(AUTHORIZE_ENDPOINT);

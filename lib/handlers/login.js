...
async function getRequestParam(req) {
  const authorizeParams = getAuthorizeParams(req, {
    ...
    iss: getEnv().APP_CLIENT_ID,
  });

  const request = await sign(authorizeParams, {
...
const authenticate = async (req, res) => {
  ...
  authorizeUrl.query = {
    client_id: getEnv().APP_CLIENT_ID,
    request: await getRequestParam(req),
  };
...
const authenticateWithRar = async (req, res) => {
  ...
  const request = await getRequestParam(req);

...
const addKeyToClientCredentials = async (
  clientId,
  publicKey,
  alg = "RS256"
) => {
...
const bootstrapProcess = async () => {
  ...
  if (!getEnv("APP_CLIENT_ID")) {
    console.log("----- Create App Client");
    const appClient = await createAppClient();
    await setEnv("APP_CLIENT_ID", appClient.client_id, { write: true });
    if (appClient.client_secret) {
      await setEnv("APP_CLIENT_SECRET", appClient.client_secret, {
        write: true,
      });
    }
  }
  ...
  if (!getEnv("APP_JWTCA_PUBLIC_KEY")) {
    ...
    await setEnv("APP_JWTCA_PUBLIC_KEY", publicKey, { write: true });
    await setEnv("APP_JWTCA_PRIVATE_KEY", privateKey, { write: true });
    await setEnv("APP_JWTCA_KEY_ID", kid, { write: true });
    await setEnv("APP_JWTCA_CREDENTIAL_ID", id, { write: true });
  }

  const appClientAuthMethod = getEnv("APP_CLIENT_AUTHENTICATION_METHOD");
  console.log({appClientAuthMethod})
  if (appClientAuthMethod === "jwtca") {
    ...
    await patchClientToUseClientSecret(getEnv("APP_CLIENT_ID"));
  }

  if (!getEnv("APP_JAR_PUBLIC_KEY")) {
    console.log("--- Generating JAR keys");
    const { publicKey, privateKey } = await generateJWTCAKeypair();
    const { kid, id } = await addKeyToClientCredentials(
      getEnv("APP_CLIENT_ID"),
      publicKey,
      getEnv("APP_JAR_KEY_ALG") || "PS256"
    );
    await setEnv("APP_JAR_PUBLIC_KEY", publicKey, { write: true });
    await setEnv("APP_JAR_PRIVATE_KEY", privateKey, { write: true });
    await setEnv("APP_JAR_KEY_ID", kid, { write: true });
    await setEnv("APP_JAR_CREDENTIAL_ID", id, { write: true });
  }
